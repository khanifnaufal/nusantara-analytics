...
<<template>
  <div class="relative min-h-screen bg-bg-dark text-text-primary p-4 sm:p-6 md:p-8">
    ...
    <div class="max-w-7xl mx-auto space-y-8 relative z-10">
      ...
      <div v-if="isInitialLoading" class="grid grid-cols-1 md:grid-cols-12 gap-6 animate-pulse">
        ...
        <div class="col-span-12 md:col-span-6 lg:col-span-5 rounded-2xl border border-white/5 bg-bg-subcard p-5 flex flex-col gap-6 h-[400px]">
        ...
        <div class="col-span-12 md:col-span-6 lg:col-span-7 rounded-2xl border border-white/5 bg-bg-subcard p-5 flex flex-col gap-6 h-[400px]">
          <div class="space-y-2">
            <div class="h-4 bg-white/5 rounded w-1/3"></div>
            <div class="h-3 bg-white/5 rounded w-1/4"></div>
          </div>
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 flex-1">
            <div v-for="i in 4" :key="'weather-skel-'+i" class="h-[140px] rounded-xl border border-white/5 bg-bg-card p-4 flex flex-col justify-between">
        ...
        <div class="col-span-12 md:col-span-12 lg:col-span-4 rounded-2xl border border-white/5 bg-bg-subcard p-5 flex flex-col gap-6 h-[380px]">
          ...
          <div class="h-[220px] rounded-xl border border-white/5 bg-bg-card p-4 flex flex-col justify-between">
        ...
        <div class="col-span-12 md:col-span-6 lg:col-span-5 rounded-2xl border border-white/5 bg-bg-subcard p-5 flex flex-col gap-6 h-[380px]">
          <div class="flex justify-between">
            <div class="space-y-2 w-1/2">
              <div class="h-4 bg-white/5 rounded"></div>
              <div class="h-3 bg-white/5 rounded w-2/3"></div>
        ...
        <div class="col-span-12 md:col-span-6 lg:col-span-3 rounded-2xl border border-white/5 bg-bg-subcard p-5 flex flex-col gap-6 h-[380px]">
          <div class="space-y-2">
            <div class="h-4 bg-white/5 rounded w-2/3"></div>
            <div class="h-3 bg-white/5 rounded w-1/2"></div>
          </div>
          <div class="space-y-3 flex-1 overflow-hidden">
            <div v-for="i in 3" :key="'quake-skel-'+i" class="flex gap-3 p-3 rounded-xl border border-white/5 bg-bg-card">
    ...
    <Transition name="fade">
      <div v-if="!isInitialLoading && globalLastUpdated" class="fixed bottom-6 right-6 z-50 flex items-center gap-2 px-3.5 py-2.5 bg-bg-card/95 hover:bg-bg-subcard border border-white/8 backdrop-blur-md rounded-full shadow-2xl text-[10px] font-semibold text-text-secondary transition-all duration-300 select-none group">
        <span class="relative flex h-2 w-2">
          <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-blue-500 opacity-75"></span>
          <span class="relative inline-flex rounded-full h-2 w-2 bg-blue-500"></span>
        </span>
